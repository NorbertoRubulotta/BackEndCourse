...
async function main() {
    ...
    async function controllerPostProductsWeb({ body }, res) {
        const newProduct = body;
        newProduct.id = randomUUID();
        await allProducts.save(newProduct);
        res.status(201);
    ...
    async function controllerGetProductsWeb(req, res) {
        const products = await allProducts.getAll();
        res.render('products', { products });
    } //funciona//


    routerWeb.post('/products', controllerPostProductsWeb)
    routerWeb.get('/products', controllerGetProductsWeb)
